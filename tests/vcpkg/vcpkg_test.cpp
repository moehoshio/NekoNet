/**
 * @file vcpkg_test.cpp
 * @brief Minimal integration test for NekoNetwork via vcpkg
 */
...
int main() {
    std::cout << "=== NekoNetwork vcpkg Integration Test ===" << std::endl;

    try {
        // Initialize network configuration
        neko::network::initialize([](neko::network::config::NetConfig& cfg) {
            cfg.setProtocol("https://")
               .setUserAgent("NekoNetwork-vcpkg-test/1.0");
        });
        ...
        std::cout << "\n=== vcpkg Integration Test Passed ===" << std::endl;
        return 0;
